...
type Props = {
  ...
  onRotateLeft: () => void; onRotateRight: () => void; onFlipHorizontal: () => void; onFlipVertical: () => void; onReset: () => void;
  onZoomChange: (value: number) => void; onZoomPreset: (value: number) => void; onRotationChange: (value: number) => void;
  ...
  return (
    <div className="canvas-toolbar" aria-label="Image controls">
      <div className="toolbar-cluster">
        ...
        <button className="toolbar-icon-button destructive-hover" onClick={onClear} title="Clear image" aria-label="Clear image"><Trash2 size={16} /></button>
      </div>
      ...
      <div className="toolbar-cluster">
        ...
        <button className="toolbar-icon-button" onClick={onFlipVertical} title="Flip vertical" aria-label="Flip vertical"><FlipVertical size={16} /></button>
      </div>
      <span className="toolbar-rule" />
      ...
      <div className="toolbar-range zoom-range">
        ...
        <div className="zoom-control-row"><button className="toolbar-mini-button" onClick={() => onZoomPreset(zoom - 0.1)} title="Zoom out" aria-label="Zoom out"><ZoomOut size={14} /></button><input aria-label="Zoom" type="range" min="0.2" max="2" step="0.01" value={zoom} onChange={(event) => onZoomChange(Number(event.target.value))} /><button className="toolbar-mini-button" onClick={() => onZoomPreset(zoom + 0.1)} title="Zoom in" aria-label="Zoom in"><ZoomIn size={14} /></button></div>
        <div className="zoom-presets"><button onClick={() => onZoomPreset(0.2)}>Fit</button><button className={Math.abs(zoom - 1) < 0.01 ? 'active' : ''} onClick={() => onZoomPreset(1)}>100%</button><button className={Math.abs(zoom - 2) < 0.01 ? 'active' : ''} onClick={() => onZoomPreset(2)}>200%</button></div>
      </div>
      <button className="toolbar-icon-button toolbar-reset-button" onClick={onReset} title="Reset all edits" aria-label="Reset all edits"><RotateCcw size={16} /></button>
    </div>
  );
}
