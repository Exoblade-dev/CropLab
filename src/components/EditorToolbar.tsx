import { FlipHorizontal, History, Redo2, RotateCcw, RotateCw, Trash2, Undo2, ZoomIn, ZoomOut, Maximize2 } from 'lucide-react';

type Props = {
  canUndo: boolean; canRedo: boolean; isHistoryOpen: boolean; zoom: number; rotation: number;
  onHistory: () => void; onClear: () => void; onUndo: () => void; onRedo: () => void;
...
export function EditorToolbar({ canUndo, canRedo, isHistoryOpen, zoom, rotation, onHistory, onClear, onUndo, onRedo, onRotateLeft, onRotateRight, onFlipHorizontal, onResetRotation, onReset, onZoomChange, onZoomPreset, onFit, onRotationChange, onRotationCommit, onRotationInteractionStart }: Props) {
  return (
    <div className="canvas-toolbar" aria-label="Image controls">
      ...
      <div className="toolbar-range rotation-range">
        ...
        <input aria-label="Rotation" aria-valuetext={`${Math.round(rotation)} degrees`} type="range" min="-180" max="180" step="1" value={rotation} onChange={(event) => onRotationChange(Number(event.target.value))} onPointerDown={onRotationInteractionStart} onPointerUp={onRotationCommit} onKeyUp={onRotationCommit} />
      </div>
      <div className="toolbar-range zoom-range">
        ...
        <div className="zoom-control-row"><button className="toolbar-mini-button" onClick={() => onZoomPreset(zoom - 0.1)} title="Zoom out" aria-label="Zoom out"><ZoomOut size={14} /></button><input aria-label="Zoom" type="range" min="0.2" max="5" step="0.01" value={zoom} onChange={(event) => onZoomChange(Number(event.target.value))} aria-valuetext={`${Math.round(zoom * 100)} percent`} /><button className="toolbar-mini-button" onClick={() => onZoomPreset(zoom + 0.1)} title="Zoom in" aria-label="Zoom in"><ZoomIn size={14} /></button></div>
        <div className="zoom-presets"><button className="toolbar-fit-button" onClick={onFit} title="Fit image in canvas" aria-label="Fit image in canvas"><Maximize2 size={11} /> Fit</button><button aria-label="Zoom to 50 percent" className={Math.abs(zoom - 0.5) < 0.01 ? 'active' : ''} onClick={() => onZoomPreset(0.5)}>50%</button><button aria-label="Zoom to 100 percent" className={Math.abs(zoom - 1) < 0.01 ? 'active' : ''} onClick={() => onZoomPreset(1)}>100%</button><button aria-label="Zoom to 200 percent" className={Math.abs(zoom - 2) < 0.01 ? 'active' : ''} onClick={() => onZoomPreset(2)}>200%</button></div>
      </div>
