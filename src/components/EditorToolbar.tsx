...
export function EditorToolbar({ canUndo, canRedo, zoom, rotation, onReplace, onClear, onUndo, onRedo, onRotateLeft, onRotateRight, onFlipHorizontal, onFlipVertical, onReset, onZoomChange, onZoomCommit, onZoomInteractionStart, onZoomPreset, onRotationChange, onRotationCommit, onRotationInteractionStart }: Props) {
  return (
    <div className="persistent-toolbar" aria-label="Persistent editor toolbar">
      ...
      <div className="toolbar-control zoom-control">
        ...
        <div className="zoom-row"><button className="toolbar-btn" onClick={() => onZoomPreset(zoom - 0.1)} title="Zoom out" aria-label="Zoom out"><ZoomOut size={15} /></button><input aria-label="Zoom" type="range" min="0.2" max="2" step="0.01" value={zoom} onChange={(event) => onZoomChange(Number(event.target.value))} onPointerDown={onZoomInteractionStart} onPointerUp={onZoomCommit} onKeyUp={onZoomCommit} /><button className="toolbar-btn" onClick={() => onZoomPreset(zoom + 0.1)} title="Zoom in" aria-label="Zoom in"><ZoomIn size={15} /></button></div>
