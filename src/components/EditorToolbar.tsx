import { FileInput, FlipHorizontal, History, Redo2, RotateCcw, RotateCw, Trash2, Undo2, ZoomIn, ZoomOut } from 'lucide-react';
...
export function EditorToolbar({ canUndo, canRedo, isHistoryOpen, zoom, rotation, onHistory, onReplace, onClear, onUndo, onRedo, onRotateLeft, onRotateRight, onFlipHorizontal, onResetRotation, onReset, onZoomChange, onZoomPreset, onRotationChange, onRotationCommit, onRotationInteractionStart }: Props) {
  return (
    <div className="canvas-toolbar" aria-label="Image controls">
      ...
      <div className="toolbar-range zoom-range">
        ...
        <div className="zoom-presets"><button className={Math.abs(zoom - 0.5) < 0.01 ? 'active' : ''} onClick={() => onZoomPreset(0.5)}>50%</button><button className={Math.abs(zoom - 1) < 0.01 ? 'active' : ''} onClick={() => onZoomPreset(1)}>100%</button><button className={Math.abs(zoom - 2) < 0.01 ? 'active' : ''} onClick={() => onZoomPreset(2)}>200%</button></div>
      </div>
