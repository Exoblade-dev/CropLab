...
export function App() {
  ...
  const outputDimensions = croppedAreaPixels
    ? getOutputDimensions(croppedAreaPixels, {
        format: exportFormat,
        quality: exportQuality,
        width: customWidth,
        height: customHeight,
  ...
  return (
    <div className={`app ${theme}`}>
      ...
      <main className="app-main">
        {!loadedImage ? (
          <UploadScreen onLoad={(file) => void loadAndReset(file)} />
        ) : (
          <div className="editor-container">
            ...
            <div className="editor-topline">
              <div>
                ...
                <h1>Crop and transform</h1>
              </div>
              <p>Adjust your image, then export a local copy.</p>
            </div>
            ...
            <div className="editor-main">
              ...
              <EditorSidebar
                ...
                zoom={cropState.zoom}
                rotation={cropState.transform.rotation}
                flipX={cropState.transform.flipX}
                flipY={cropState.transform.flipY}
                lockAspectRatio={lockAspectRatio}
                onAspectChange={(value, label) => {
                  saveState(cropState);
                  setSelectedAspect(value);
                  showToast(`Aspect ratio set to ${label}`);
                }}
                onZoomChange={handleZoom}
                onZoomCommit={() => saveState(cropState)}
                onRotationChange={handleRotation}
                onRotationCommit={() => saveState(cropState)}
                onFlip={flip}
      ...
      <footer className="app-footer">
        <div className="footer-content">
          <div className="footer-brand"><strong>CropLab</strong><span>Private by design · runs entirely in your browser</span></div>
          <div className="footer-meta"><span>JPEG · PNG · WebP · GIF</span><span>© {new Date().getFullYear()} CropLab</span></div>
        </div>
      </footer>
    </div>
  );
